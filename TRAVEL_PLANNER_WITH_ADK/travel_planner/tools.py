...
def web_search(query: str) -> str:
    ...
    recency_words = ["latest", "recent", "current", "new", "developments", "update", "2026"]
    if any(w in query.lower() for w in recency_words) and "2026" not in query:
        query = f"{query} 2026"
    with DDGS() as ddgs:
...
_search_agent= Agent(
    ...
    instruction= """
        TODAY'S DATE: April 23, 2026. Always treat 2026 as the current year when searching or answering.
        ...
        When searching for latest/recent/current information, always include "2026" in your search query.
        IMPORTANT:
        - Always return your response in bullet points
        - Specify what matters to the user
        - Do not reference 2025 as the current year; it is 2026
    """,
    tools=[google_search]
)
