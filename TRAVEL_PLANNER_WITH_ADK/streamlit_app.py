...
# Load environment variables from root .env and travel_planner/.env
load_dotenv()
...
TODAY'S DATE: April 23, 2026. Always use this as the reference for "current", "latest", "recent", or "new" information.
...
- When mentioning recent developments, events, or news, always refer to 2026 as the current year
...
class SimpleTravelAgent:
    ...
    async def run(self, user_message: str, conversation_history: list = None) -> str:
        ...
        try:
            # Initialize contents with system prompt
            contents = [
                Content(
                    role="user",
    ...
    async def _check_and_use_tools(self, message: str) -> str:
        ...
        if any(keyword in message_lower for keyword in location_keywords):
            ...
            if location:
                ...
                if query:
                    try:
                        result = location_search_tool.func(query, location)
        ...
        search_keywords = ["events", "news", "current", "happening", "trending", "festival", "what's",
                           "latest", "recent", "developments", "new", "update", "2026"]
        if any(keyword in message_lower for keyword in search_keywords):
            try:
                result = google_search_grounding.agent.run(message)
